...
if ($et.timeline) {
  ...
  window.addEventListener('scroll', function(e) {
    if (enablePreloading && !loading && nearingBottom()) {
      ...
      if (moreButton.length) {
        loading = moreButton.attr('href')
        var pageNumber = Number(loading.match(/\bpage=(\d+)/)[1])
  ...
  jQuery($et.sidebar).bind('ajaxSuccess', function(e, xhr, ajax){
    ...
    if (url.path == '/timeline/render_following_avatars') sortFriends()
    else if (ajax.url == loading) {
      loading = false
    } else if (dynamicPages.indexOf(url.path) != -1) {
      enablePreloading = true
      // remove the pagination message which might be stale
      var msg = $('pagination-message')
      if (msg) removeChild(msg)
  ...
  content.addEventListener('click', function(e) {
    ...
    if (link && /^\s*in reply to /.test(link.textContent)) {
      ...
      loadJSON(statusUrl, function(response, xhr) {
        if (xhr.status >= 400) { fallback.call(xhr); return }
